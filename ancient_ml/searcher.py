...
from .feature_builder import cheap_predict
from .registry import add_candidate, connect
...
def random_seed(rng: random.Random, year_min: int = -5000, year_max: int = 5000) -> SyntheticSeed:
    ...
    return SyntheticSeed(
        ...
        latitude=rng.uniform(-66.0, 66.0),
...
def mutate_seed(seed: SyntheticSeed, rng: random.Random, scale: float = 1.0) -> SyntheticSeed:
    return SyntheticSeed(
        ...
        latitude=max(-66.0, min(66.0, seed.latitude + rng.uniform(-3.0 * scale, 3.0 * scale))),
        longitude=((seed.longitude + rng.uniform(-6.0 * scale, 6.0 * scale) + 180.0) % 360.0) - 180.0,
        ayanamsha=seed.ayanamsha,
    )


def evaluate_seed(seed: SyntheticSeed, train_draws: list[Draw], validation_draws: list[Draw]) -> dict:
    ...
    total = train_score["points"] + 2.0 * val_score["points"]
    return {
        "train": train_score,
...
def search_once(draws: list[Draw], state_path: str, trials: int, top_k: int, rng_seed: int | None = None) -> None:
    ...
    for i in range(1, trials + 1):
        ...
        try:
            result = evaluate_seed(seed, train_draws, validation_draws)
        except Exception:
